...
class HttpRule(betterproto.Message):
    ...
    populated from the path, query parameters, or body of the HTTP request.
    The mapping is typically specified as an `google.api.http` annotation, see
    ...
    2;    // `sub.subfield` is url-mapped } message Message {   string text =
    1; // content of the resource } ``` This definition enables an automatic,
    ...
    the message resource collection: ```proto service Messaging {   rpc
    ...
    encoding: HTTP | RPC -----|----- `PUT /v1/messages/123456 { "text": "Hi!"
    ...
    rarely used in practice of defining REST APIs. The common usage of `*` is
