...
class ScalarDescriptor(betterproto.Message):
    ...
    legacy_amino_encoding: str = betterproto.string_field(4)
    """
    legacy_amino_encoding is an optional string to describe the encoding format
    used by Amino. The field type is chosen to be a string so that the value
    can either be: - a machine-readable string, such as "base64", "bech32" or
    "utf8", - or a human-readable string, for instance a short specification of
    how a big integer would be encoded using Amino. If left empty, then the
    Amino encoding is expected to be the same as the Protobuf one.
    """
